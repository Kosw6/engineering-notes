# JMC를 활용한 Allocation 기반 성능 병목 분석
...
1. [Summary](#-summary)
2. [테스트 환경](#테스트-환경)
3. [지난 테스트 Review](#지난-테스트-review)
4. [문제 배경 (JMC 도입 이유)](#jmc를-활용하게된-주요-문제-배경)
5. [분석 접근 방법](#분석-접근-방법)
6. [주요 발견 사항](#주요-발견-사항)
...
7. [V1 → V2 개요](#v1v2)
8. [문제 분석 및 원인](#v1의-분석-및-문제점-해결방안)
9. [코드 개선 (JWT Hot Path)](#v1-v2-hotpath-변경-코드-클릭하여-보기)
10. [성능 개선 결과 (k6)](#수정-전후-k6부하테스트-결과-및-jfr분석-결과)
11. [JMC 분석 비교](#v1v2-jmc이미지-분석)
12. [V1 → V2 결론](#v1v2결론)
...
13. [2step 구조 변경 배경](#v22step-조회-변경사항)
14. [코드 변경 (2step 조회)](#v2-2step조회-변경-코드-클릭하여-보기)
15. [성능 결과 비교 (k6)](#k6부하테스트-결과-및-jmc이미지를-통한-분석)
16. [JMC 분석 (병목 이동)](#2step이미지를-볼-경우)
17. [GC 분석](#gc-summary-최종-비교-본부하-기준)
18. [2step 최종 분석](#2step-최종-분석)
...
19. [최종 구조 선택 (V2 확정)](#gc-summary-최종-비교-본부하-기준)
20. [일반 JRE 환경 재검증](#최종-일반-jre-환경에서-재검증-결과)
21. [SLO 기준 검증](#개선-후-slo-기준-검증)
22. [최종 분석 및 해석](#분석-및-해석)
...
## 🎯 문제 정의
트래픽 증가 시 Node 조회 API의 p95 latency가 상승하며  
SLO(p95 ≤ 300ms)를 만족하지 못하는 구간이 발생했다.  

해당 API는 그래프 기반 조회에서 핵심적으로 사용되며,  
사용자가 여러 그래프를 바꿔가며 조회하는 UX 특성상  
낮은 지연(latency)이 중요한 엔드포인트이다.  

또한 그룹 기반 기능에서는 다수 사용자가 동시에 조회를 수행하기 때문에  
지연 증가는 곧 전체 사용자 경험 저하로 이어질 수 있는 구조였다.  

기존 Fetch Join 기반 최적화 이후에도  
추가 개선 방향이 불명확한 상태였다.

---

## 🔍 핵심 원인
JFR/JMC 기반 분석 결과, 성능 병목은 쿼리가 아닌  
공통 인증 경로(Hot Path)에 존재하고 있음을 확인했다.

- JWT 검증 로직 중복 실행
- 인증 과정에서의 불필요한 DB 조회
- Base64 decode 과정에서 byte[] / char[] 과다 생성

이로 인해 Allocation 증가 및 GC Pause가 발생하며  
tail latency를 악화시키고 있었다.

---

## 🛠 해결 전략
- JFR + JMC 기반 런타임 분석을 통해 병목 위치 식별
- JWT 검증 1회화 및 검증 결과 재사용
- 인증 과정에서 DB 조회 제거 (stateless 인증)
- Hot Path 제거를 통한 Allocation 감소

또한, 구조적 개선 가능성을 검증하기 위해  
2-step 조회 구조 PoC를 수행하였다.

---

## 📈 결과
- p95 latency: **312ms → 110ms (약 65% 개선)**
- Old GC Time: **3.47s → 2.22s (약 36% 감소)**
- Allocation 및 동시성 대기(ConditionNode) 감소

→ 지난 테스트 기준 동일 부하에서 SLO를 안정적으로 만족
...
문제 해결의 타당성을 검증하기 위해,
동일 부하 조건에서 구조 변경 전후를 비교하였다.

Hot Path 개선 이후:
- p95 latency가 312ms → 110ms로 감소
- Old GC Time 감소 및 Allocation 감소 확인

이는 분석 단계에서 식별한 병목(인증 경로 Allocation 및 GC 증가)이  
실제 성능 저하의 주요 원인이었음을 뒷받침한다.

또한 2-step 구조를 추가로 검증한 결과,
DB/JDBC 병목은 완화되었으나
객체 그래프 조립 비용 증가로 인해 성능이 오히려 악화되었고,

이를 통해 구조 변경보다
핵심 병목 제거가 우선이라는 판단의 타당성을 확인하였다.

> 단순 성능 개선이 아니라,
원인으로 식별된 병목을 제거했을 때
예상된 방향으로 결과가 변화함을 확인함으로써
해결의 타당성을 검증하였다.

## 💡 핵심 인사이트
- 성능 병목은 쿼리가 아니라 **공통 Hot Path에 존재**
- 구조 확장보다 **핵심 병목 제거가 우선**
- 2-step 구조는 DB 병목을 줄였지만  
  객체 그래프 조립 비용 증가로 인해  
  **병목이 애플리케이션 레벨로 이동**

→ 최종적으로 V2(fetch join + hotpath 개선)를 선택
...
## 최종 일반 JRE 환경에서 재검증 결과

| 구분       | RPS    | p95(ms) | 평균 처리량(req/s) | 실패율   |
| -------- | ------ | ------- | ------------- | ----- |
| 개선 전(V1) | 30→105 | 312.38  | 105.01        | 0.00% |
| 개선 후(V2) | 30→105 | 110.95  | 105.01        | 0.00% |
...
| ------- | ----- | -------- |
| 105 RPS | 110ms | 여유       |
| 130 RPS | 239ms | ✅ SLO 만족 |
| 150 RPS | 400ms | ❌ SLO 초과 |

---

## 분석 및 해석

* JFR/JMC 분석을 통해 식별된 인증 경로의 **불필요한 allocation 및 중복 검증 병목을 제거**한 후, 동일 부하(30→105 RPS) 기준 일반 JRE 환경에서 재측정한 결과 p95가 약 **312ms → 110ms** 수준으로 감소하였다.
  이는 요청 처리 과정에서 발생하던 **객체 생성 비용 및 GC 부담 감소**에 따라 tail latency가 개선된 결과로 해석된다.

* JFR 계측 환경은 이벤트 기록 및 샘플링 오버헤드로 인해 절대 지연이 증가할 수 있으므로, 본 실험에서는 JFR을 병목 원인 식별 용도로 활용하고 최종 성능 평가는 일반 실행 환경(JRE)에서 재검증하였다.

* 또한 부하를 점진적으로 증가시키며 측정한 결과, **130 RPS까지는 SLO(p95 ≤ 300ms)를 안정적으로 만족**하였으나, **150 RPS 구간에서는 p95가 급격히 증가**하며 SLO를 초과하였다.
