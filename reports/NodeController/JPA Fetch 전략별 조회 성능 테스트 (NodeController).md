...
각 전략이 **쿼리 수, 응답 크기, 메모리 사용, GC 영향**에 어떤 차이를 만드는지 실측 기반으로 분석한 결과를 정리합니다.

## 전제 조건

> 본 문서의 성능 비교 결과는 동일한 요청 패턴과 데이터 분포에서  
> JPA 조회 전략(Lazy, Batch Fetch, Fetch Join, Projection, Native Query + JSON Aggregation)을 비교한 결과입니다.

> 따라서 본 문서의 핵심 비교 대상은 단순 SQL 실행 시간 자체가 아니라,  
> 왕복 쿼리 수, 결과 행 수(Row Explosion), 객체 생성량, JSON 직렬화 비용, GC(STW)까지 포함한  
> 애플리케이션 레벨의 전체 조회 비용입니다.

> 특히 Fetch Join의 성능 우위는 단순히 쿼리 수 감소 때문만이 아니라,  
> Hibernate 1차 캐시 기반의 부모 엔티티 Deduplicate로 인해  
> Projection 대비 객체 생성량과 GC 부담이 줄어든 결과로 해석하였습니다.

> 또한 본 결과는 현재의 데이터 분포(노드 10개 × 링크 10개 수준),  
> 목록 조회 중심의 요청 패턴, 그리고 페이징이 필요하지 않은 화면 완결형 조회를 전제로 합니다.

> 따라서 실제 운영 환경에서는 데이터 개수, 연관관계 크기, 페이징 필요 여부,  
> 응답 페이로드 크기, JVM/GC 상태에 따라 최적 전략이 달라질 수 있습니다.
...
`node_note_link` 테이블을 **Lazy 로딩**으로 긁어오면서 N+1 쿼리가 쏟아지고 있었습니다.

그래서 JPA에서 흔히 쓰는 세 가지 접근 — **Lazy / Batch Fetch / Fetch Join** — 을 전부 돌려보기로 했습니다.
각 전략을 동일 부하(120RPS, seed=777)로 3회씩 테스트했고,
결국 **Fetch Join**이 왕복 쿼리를 최소화하면서 p95가 가장 낮게 나왔습니다.

추가로 PostgreSQL의 **`work_mem`** 값(8MB → 128MB)을 조정해봤는데,
생각보다 영향이 없었다. 해시나 정렬이 아니라 쿼리 패턴 자체가 병목이라
디스크 스필도 DB레벨에서 확인결과 안 생겼고, 단순히 왕복 횟수가 성능을 결정하고 있었습니다.

---

### 2차 테스트(Node,Node_Note_link,Note)

1차에서 성능은 잡혔지만, UI 쪽 요구가 생겼습니다.
노드 목록에서 이제는 `noteId`만이 아니라 **`noteSubject`(제목)** 도 같이 내려줘야 했습니다.
이때 “행 폭증” 문제가 눈에 들어왔으며 Node 10개에 링크 10개씩만 붙어도
조인 결과가 100행이 되는 구조였습니다.

그래서 **JSON Aggregation (`json_agg` + `GROUP BY`)** 을 써서
DB 단에서 한 번에 묶는 방식을 시도했으며
`EXPLAIN (ANALYZE)` 로 봤을 땐 잘 돌아갔고, 행 수도 확실히 줄었지만
막상 k6 부하를 걸어보니 p95가 오히려 더 늘었습니다.
이를 통해 집계,정렬,직렬화 CPU가 무거운걸로 확인했습니다.

결국 이 방법 대신에, 아예 **스키마 쪽을 손보는 방향으로 전환**했습니다.
...
`note.subject`가 바뀔 때 자동으로 싱크되도록 **DB 트리거**를 달았습니다.
이렇게 하면 애플리케이션 쪽 코드를 거의 안 건드리고도 제목을 바로 조회할 수 있습니다.

또 하나 떠올랐던 아이디어가 “본문을 짧게 잘라서 보내면 더 빨라지지 않을까?”였고
서비스 상 노드 본문은 어차피 화면에서 짧게 보여주니까,
서버에서 미리 **`substring(content, 1, 20)`** 으로 자른 프리뷰만 내려주는 테스트를 추가했습니다.

여기서부터는 조회 방식도 나눴습니다.
엔티티를 그대로 조립하는 **Fetch Join**,
필요한 필드만 딱 받는 **Projection**,
그리고 이전에 썼던 **Native + JSON Aggregation**.
총 네 가지 조합으로 테스트를 진행했습니다.
...
성능은 대체로 다음 순서로 좋았습니다.

**JSON Aggregation < 500자 Projection < 20자 Projection < 500자 Fetch Join**

추가로
GC·스레드·커넥션·p95/p99를 전부 모니터링하면서 분석하였고

결론적으로, 둘 다 DB에서는 100행을 읽지만
**Fetch Join은 Hibernate가 1차 캐시 기준으로 부모(노드)를 Deduplicate** 합니다.
즉, 같은 부모 엔티티가 여러 번 나와도 새로 객체를 안 만들고,
자식(링크)만 컬렉션에 추가합니다.
반면 Projection은 **DTO 100개를 전부 새로 만듭니다.**

결과적으로 힙에 올라가는 객체 수가 10배 차이 나고,
GC에서 **Fetch Join의 Pause가 평균 5ms**, Projection은 **6ms** 정도였으며
이 차이가 결국 p95까지 이어졌습니다.
요약하면, 행 폭증 상황에서는 **Fetch Join이 메모리 효율과 GC 안정성 면에서 더 낫다**는 걸 확인했습니다.

추가로 20자/500자 Projection 비교에서는 GC 차이는 크지 않았고,
20자 반환의 p95 개선은 주로 **응답 페이로드 축소와 JSON 직렬화/역직렬화 비용 감소** 영향으로 해석하였습니다.

### 4차 테스트(Node, Node_Note_Link, Note)

3차 테스트를 통해 목록 조회 시 Fetch Join 기반 조회가 가장 낮은 p95를 보인다는 것을 확인했습니다.
다만 500자/20자 Projection 비교에서 GC Pause 차이는 크지 않았고, 오히려 p95 차이는 직렬화·페이로드 처리 비용의 영향일 가능성이 있다고 판단했습니다.
이에 본문(content) 크기가 GC·스레드·tail latency에 미치는 영향을 명확히 보기 위해, 노드 content를 `한글 기준 약 1만자(≈30KB)`로 확대한 뒤 추가 비교 실험을 진행했습니다.
...
동일한 Fetch Join 기반 조회 흐름에서, content 처리 위치만 달리하여 3가지 케이스로 비교했습니다.
...
- content 원문(1만자)을 애플리케이션 레벨로 가져오는 케이스(2,3) 에서는
  ...
  동일한 p95 구간에서 유지 가능한 RPS가 DB 레벨 프리뷰(1) 대비 약 5배 낮게 나타났습니다.

- 특히 원문을 그대로 반환하는 케이스는 특정 RPS 구간에서 GC Pause가 크게 증가하며 p95가 급격히 붕괴하는 현상이 관찰되었습니다.

- 반면 APP 레벨 프리뷰(3)는 원문 그대로 반환(2) 대비, 동일 RPS(예: 26 RPS)에서 붕괴가 발생하더라도
  붕괴 빈도가 낮고 상대적으로 더 안정적인 패턴을 보였고
  이는 본 테스트에서 지배적인 원인은 GC/할당 압력이지만, 동시에 응답 JSON 직렬화 비용 역시 tail latency에 유의미한 영향을 준다는 근거로 해석할 수 있습니다.

해석 및 결론

해당 실험을 통해 목록 조회처럼 요청 수가 많은 구간에서 대용량 본문을 애플리케이션으로 가져오는 것은
GC(STW)와 스레드 지연을 유발하여 tail latency(p95)를 빠르게 붕괴시키는 주요 원인임을 확인했습니다.

따라서 최종적으로 목록 조회에서는 DB 레벨에서 프리뷰(20자)를 생성해 전달하고,
원문은 상세 조회에서만 Lazy Loading으로 분리하는 구조가 가장 합리적인 선택이라고 결론 내렸습니다.

또한 APP 레벨 프리뷰가 일부 안정성을 보인 점을 통해, GC 영향이 더 크지만 JSON 직렬화 비용도 무시할 수 없다는 점을 함께 확인했습니다.
