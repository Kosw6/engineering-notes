...
### nvcswch VS cswch 요약 설명

>nvcswch는 CPU 경쟁 상황에서 스케줄러에 의해 강제로 발생하는 context switching으로,
  Thread 부족으로 runnable queue가 증가할 경우 급격히 증가하는 특징을 보인다.

> 반면 cswch는 DB I/O, lock 대기 등으로 thread가 스스로 CPU를 양보할 때 발생하는 자발적 context switching이다.

- 해당 실험에서는 Thread가 부족한 경우 runnable queue 경쟁으로 인해
  nvcswch(involuntary context switching)가 급증하는 패턴이 관찰되었다.

- 반면 Thread가 과도하게 많은 경우에는 CPU 코어 수 대비 활성 thread 수가 증가하면서
  scheduling overhead가 커지고, thread 간 양보가 잦아지며 cswch가 증가하는 경향을 보였다.

- 또한 Hikari pool 크기가 적절하지 않은 경우에도 cswch 증가가 관찰되었다.<br>
  1. pool이 작은 경우에는 제한된 커넥션에 접근이 집중되면서 borrow/return 과정에서의 상태 변경 및 동기화 작업이 증가하고,
  이로 인해 일부 thread가 잠시 대기 상태로 전환되면서 cswch가 증가하는 경향을 보였다.
  2. pool이 큰 경우에는 동시에 활성화되는 thread 수가 증가하면서 DB I/O 대기 구간이 늘어나고,
  thread가 자발적으로 CPU를 양보하는 상황이 많아지면서 cswch가 증가하는 경향을 보였다.
...
> Thread 부족 → runnable queue 경쟁 → **강제 context switching 증가 → 성능 붕괴**<br>
> 해당 실험에서 Thread 4 구간은 saturation 진입 경계로 판단되며,<br>
> Thread 2에서는 runnable queue 경쟁으로 인해 특히 nvcswch(involuntary context switching)이 급증하며
> 시스템이 saturation 상태에 진입한 것으로 확인되었다.

해당 실험에서 saturation은 CPU 자원 한계로 인해 thread 간 경쟁과 대기가 발생하고, 그 결과 latency가 급격히 증가하는 상태로 정의하였다.
...
스레드 수를 증가시키면 동시에 더 많은 작업을 처리할 수 있을 것처럼 보이지만,
CPU 코어 수보다 많은 runnable thread가 존재할 경우 운영체제 스케줄러는
각 스레드에 CPU를 번갈아 할당하게 된다.

이 과정에서 기존 스레드의 실행 상태를 저장하고
다음 스레드의 실행 상태를 복원하는 작업이 발생하는데,
이를 컨텍스트 스위칭(context switching)이라고 한다.

컨텍스트 스위칭 시에는 register, program counter, stack pointer 등의 실행 문맥을
저장하고 복원하는 오버헤드가 발생하며,
스레드 수가 증가할수록 이러한 스케줄링 비용 역시 함께 증가하게 된다.

특히 본 실험과 같이 DB I/O가 포함된 workload에서는
많은 스레드가 동시에 실행되더라도 실제로는 DB 응답을 기다리는 대기 상태에 놓이는 경우가 많다.
이 경우 CPU는 유의미한 작업을 수행하기보다
대기 중인 스레드 간 전환에 시간을 소비하게 되며,
결과적으로 전체 latency가 증가하는 현상이 발생한다.

따라서 thread 수를 단순히 늘리는 방식은 항상 처리량 증가로 이어지지 않으며,
CPU 코어 수와 workload 특성을 고려하여 적절한 수준으로 제한하는 것이
더 안정적인 성능을 만드는 방향이라고 판단했다.

## DB Connection Pool Pending과 병목 연결

스레드 수가 증가하면 동시에 DB에 접근하는 요청도 함께 증가한다.
DB connection pool은 제한된 자원이기 때문에,
최대 pool size를 초과하는 요청은 pending 상태로 대기하게 되고
전체 latency 증가로 이어질 수 있다.

다만 본 실험에서는 pending 상태는 관찰되지 않았으며,
현재 워크로드 기준으로 DB 처리 여유가 존재한 것으로 확인되었다.

따라서 특정 구간 이후 latency가 다시 증가한 주요 원인은
DB 병목이 아니라 CPU 레벨에서의 컨텍스트 스위칭 비용 증가로 판단된다.

단, 트래픽 증가 또는 DB 처리 여유 감소 상황에서는
동일 구조에서 connection pool pending이 병목으로 전환될 수 있으며,
이 경우 latency 악화는 더욱 크게 나타날 수 있다.

결론적으로 thread pool과 DB connection pool은 독립적으로 확장하는 것이 아니라,
vCPU, DB 처리 능력, workload 특성을 함께 고려하여
균형 있게 조정해야 한다.


>정리하면, thread 수 증가가 항상 성능 향상으로 이어지지 않았던 이유는<br>
CPU 코어 수를 초과하는 runnable thread 증가에 따른 컨텍스트 스위칭 비용이 발생하였으며,<br>
쓰레드 수 증가나 트래픽 확대 상황에서 DB connection pool 한계로 인해 발생하는 pending 대기가 함께 작용할 가능성이 높다.

>따라서 본 설정은 단순 최대 처리량을 높이기 위한 목적이 아니라,<br>
현재 시스템 자원(vCPU, DB connection, workload 특성)을 고려하여<br>
안정적인 latency를 유지하기 위한 유량제어 관점에서 도출된 값이다.
