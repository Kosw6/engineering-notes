...
![architecture](../image/observability/Observability.png)
...
#### ❌ OpenTelemetry Collector 기반 수집 구조

- 장점: Trace / Metric / Log를 하나의 표준 수집 파이프라인으로 통합 가능
- 단점: 초기 설정 복잡도 증가, Collector 파이프라인 설계 필요, 현재 목적 대비 과함

해당 시스템에서는 분산 트레이싱보다
로그 기반 ERROR 감지와 서버 리소스 관측이 핵심이었다.

따라서 로그 수집은 Promtail → Loki,
메트릭 수집은 Node Exporter → Prometheus 구조로 단순하게 구성하였다.

향후 서비스 간 호출 추적이나 분산 트레이싱이 필요해질 경우,
OpenTelemetry Collector 도입을 검토할 수 있다.
...
## 3.4 eBPF 기반 커널 레벨 관측 확장

### 3.4.1 도입 배경

기존 시스템은 애플리케이션 로그와 메트릭을 통해 장애를 감지할 수 있었지만,

* 장애 발생 원인의 계층 (네트워크 / 디스크 / 메모리) 구분이 어려움
* 시스템 레벨 병목에 대한 직접적인 관측 불가
...
이를 해결하기 위해
**커널 레벨에서 발생하는 이벤트를 직접 관측하기 위한 eBPF 기반 지표를 추가하였다.**

---

### 3.4.3 수집 지표

* TCP Retransmit (네트워크 이상 감지)
* Context Switch (스케줄링 상태)
* OOM Kill (메모리 부족)
* Block I/O Latency (디스크 병목)
...
![ebpf-dashboard](../image/observability/ebpf_defalut.png)

#### 📌 구성 개요

본 대시보드는 eBPF 기반 커널 신호를 서비스별로 구분하여 통합 구성한 장애 원인 분석 뷰이다.

App, DB, Kafka, Redis 각 컴포넌트에 대해
네트워크(TCP retransmit), 스케줄링(Context switch), 메모리(OOM), 디스크(I/O latency) 지표를 분리하여 배치하였다.

이를 통해 장애 발생 시 단순 증상 확인이 아닌,
각 계층별 병목 원인을 빠르게 검증할 수 있도록 설계하였다.
...
- DB eBPF Signals  
  - ext4 평균 latency  
  - ext4 read p95 latency  
  - I/O throughput  

- Kafka eBPF + Resource Signals  
  - TCP retransmit  
  - OOM Kill  
  - CPU / Memory 사용량  

- Redis eBPF + Resource Signals  
  - TCP retransmit  
  - CPU / Memory 사용량  
...
본 대시보드는 장애 발생 시 **어디에서 문제가 발생했는지**와
**왜 발생했는지**를 동시에 파악할 수 있도록 설계하였다.

---

### 3.4.6 활용 방식

| 대시보드 패널 | 의미 | 가능한 원인 | 다음 확인 |
|--------------|------|------------|----------|
| App: TCP Retransmit to DB ↑ | DB 연결 재전송 증가 | DB 네트워크 지연, 연결 불안정 | DB latency, connection pool |
| App: TCP Retransmit 전체 ↑ | 전체 네트워크 재전송 증가 | 인스턴스 네트워크 문제 | Kafka/Redis retransmit |
| App: Context Switch ↑ | 스케줄링 증가 | thread 과다, CPU contention | CPU usage, thread pool |
| App: OOM Kill 발생 | 프로세스 종료 | 메모리 부족 | JVM heap, container memory |
| DB: ext4 Avg Latency ↑ | 디스크 평균 지연 | I/O 병목 | query time, disk usage |
| DB: ext4 Read p95 ↑ | 디스크 tail latency 증가 | 특정 쿼리 지연 | slow query |
| DB: I/O Throughput 변화 | 디스크 사용량 변화 | read/write 병목 | DB load |
| Kafka: TCP Retransmit ↑ | Kafka 네트워크 문제 | broker 네트워크 지연 | consumer lag |
| Kafka: OOM Kill | Kafka 프로세스 종료 | 메모리 부족 | broker memory |
| Redis: TCP Retransmit ↑ | Redis 연결 지연 | 네트워크 문제 | Redis ops/sec |


해당 표는 단순 지표 설명이 아니라,
대시보드를 기반으로 장애 원인을 빠르게 검증하기 위한 운영 가이드로 활용된다.
...
## 🔜 다음 단계: 장애 대응 및 복구 전략으로 확장

>해당 관측 체계는 향후 장애 대응 시,
로그 기반 감지에 더해 eBPF 기반 커널 신호와 시스템 리소스 지표를 함께 활용하여
병목 원인을 검증하고, 상황에 맞는 대응 전략을 선택하는 데 활용할 예정이다.

다음 단계에서는 해당 관측 체계를 기반으로  
실시간 협업 시스템에서 발생할 수 있는 장애에 대해  
다음과 같은 대응 전략을 설계 및 검증할 예정이다.

- Redis 장애 시 상태 저장 실패에 대비한 로컬 메모리 및 DB 기반 fallback 처리
- Kafka 장애 시 Degraded Mode 전환 및 이벤트 임시 적재 후 복구 시 Replay 수행
- Redis Pub/Sub 장애 시 Kafka 또는 polling 기반 보조 전파
- WebSocket 서버 장애 시 Gateway 기반 failover 및 세션 복구
- DB 장애 시 Circuit Breaker 적용 및 PENDING 상태 기반 쓰기 지연 처리

---

이를 통해

- **장애 시 서비스는 지속적으로 유지하고 (Degraded Mode)**  

- **복구 이후 Replay 및 Version 기반으로 정합성을 회복하는 구조**로 확장한다.
- 추가로 장애 상황에서 **Degraded Mode 전략을 적용할 때**,<br>
  **eBPF 및 시스템 리소스 지표를 활용**하여
  상태 저장 방식과 이벤트 전파 방식 간의 **자원 사용 및 지연 특성을 비교**하고,
  **서비스 특성에 맞는 최적 전략을 검증**할 예정이다.
