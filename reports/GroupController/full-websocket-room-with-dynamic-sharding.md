...
하나의 서버에 fanout 부하가 집중되는 상황이 발생할 수 있다.

이 경우 단순한 그룹 기반 샤딩만으로는
대형 room에서 발생하는 broadcast 부하를 완전히 완화하기 어렵다.
...
> **write 요청은 owner 서버 한 곳으로 모아 직렬화**
...
단 해당 오너 서버를 통한 중앙 직렬화는 서비스 기능 중 오직 한 명만 가능한 락 점유 상황에서 사용하며<br>
설계상 여러명이 작업이 가능한 노드 수정 이벤트는 Redis Draft + DB Entity Version를 활용하여 검증하기에 <br>쓰기 요청을 각 서버에서 진행하여 불필요한 중앙 집중, 서버 부하 증가, 반응성 저하등의 비용을 방지한다.
