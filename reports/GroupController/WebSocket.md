# WebSocket 실시간 협업 시스템 설계 개선 과정

# 🚀 Summary

## 🎯 문제 정의
실시간 협업 환경에서 WebSocket 브로드캐스트 시  
RAW 구조에서 메시지 유실 및 세션 붕괴가 발생하고,  
대부분의 메시지가 1000ms 이상 지연되며 실시간성이 붕괴되었다.

---

## 🔍 붕괴 원인
- 동일 WebSocketSession에 대한 동시 send → TEXT_PARTIAL_WRITING 예외
- 예외 발생 후 세션 제거 → 브로드캐스트 범위 자체 붕괴
- All-delivery 구조로 인한 큐 적체 및 지연 누적

---

## 🛠 1차 해결 (동시성 안정화)
- ConcurrentWebSocketSessionDecorator 적용
- 세션 단위 send 직렬화 + 버퍼링

→ 결과:
- 수신량 6,000 → 2,400,000 회복
- fan-out 처리 정상화

---

## ⚠️ 구조적 한계 (실시간성 붕괴)
- 모든 이벤트를 전송(All-delivery)하면서
  버퍼링 및 직렬화로 인해 지연이 누적
- ≤200ms 비율이 0~5% 수준으로 붕괴

---

## 🛠 2차 해결 (전송 전략 전환)
- Drop / Latest-only 전략 도입
- 휘발성 이벤트는 최신 상태만 유지하도록 변경
...
## ❗ 추가 문제 (중복 flush)
- 동일 메시지가 주기적으로 반복 전송됨
- 불필요한 fan-out 증가 → CPU / 네트워크 낭비
- tail latency 왜곡

---

## 🛠 최종 해결 (Dirty Flag)
- 변경된 경우에만 flush 수행 (dirty-drain)
- 중복 전송 제거 및 fan-out 안정화

→ 결과:
- ≤200ms 비율 **99% 이상 회복**
- CPU / Memory / Allocation 안정화
...
- Drop 전략만으로는 부족하며,  
  **중복 전송 제어(Dirty Flag)가 구조 안정성의 핵심**
- 성능 문제의 본질은 처리량이 아니라  
  **불필요한 fan-out과 전송 구조**

또한 동일 조건에서 STOMP와 RAW 구조를 비교한 결과,  
RAW 구조가 CPU 사용률이 낮고 실시간 응답률이 더 높아  
현재 요구사항(저지연 브로드캐스트)에 더 적합하다고 판단하였다.
...
1. [문제 발생 (RAW 동시성)](#2-문제-발생-raw-동시성)
2. [원인 분석](#원인-분석)
3. [해결 (Decorator)](#raw-동시성-해결)
4. [안정성 확보 결과](#raw-동시성-문제-개선-후-결과)
5. [그러나 실시간성 붕괴 발견](#수신-메세지-지연과-구조적-한계)
6. [Drop 전략 도입](#1차-개선-drop-only-latest)
7. [성능 개선 결과](#더티-플래그-추가-후-테스트-결과)
8. [RAW vs STOMP 비교 (JFR 근거)](#최종-stomp-vs-raw-비교)
9. [최종 결론 및 트레이드오프](#결론)
