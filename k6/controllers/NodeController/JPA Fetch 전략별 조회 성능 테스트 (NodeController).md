...
실험을 진행함에 따라서 초기에는 DB 쿼리(N+1)가 주요 병목이었으나, 이를 해결한 이후에는 객체 생성 및 GC, 최종적으로는 응답 페이로드와 직렬화 비용이 tail latency를 결정하는 주요 요인으로 작용하는 것을 확인했다.
...
| 구분                                                     | RPS    | P95(ms)            | 평균 처리량(req/s, 3회 평균) | 실패율 |
| -------------------------------------------------------- | ------ | ------------------ | ---------------------------- | ------ |
| **1만자 콘텐츠 -> 20자(DB) Fetch Join**                  | 30→120 | 860.23 → 586.61    |    **120.01**                       | 0.00%  |
...
- 직렬화 관련 분석에서, 애플리케이션 레벨에서 20자로 축소하는 경우 원문 반환 대비 p95 latency와 tail 구간 안정성이 개선되는 것을 확인했다.
이는 대용량 payload를 그대로 직렬화하는 과정에서 발생하는 비용이 감소한 영향으로 해석할 수 있다.

- 다만 1만자 payload를 애플리케이션으로 가져오는 두 가지 테스트(APP 프리뷰, 원문 반환)의 경우, 약 26 RPS 구간에서 GC Pause 증가와 함께 p95가 급격히 붕괴되는 현상이 관찰되었다.

- 세 가지 테스트를 종합해보면, 본 환경에서는 직렬화 비용보다도 대용량 데이터를 객체로 생성하는 과정에서 발생하는 할당 증가와 GC 압력이 더 지배적인 병목으로 작용한 것으로 판단된다.

- 즉, 직렬화 비용 역시 tail latency에 영향을 주는 요소이지만, 시스템이 붕괴되는 임계 구간에서는 GC로 인한 STW(Stop-The-World)가 주요 원인으로 작용하며, 두 요소가 복합적으로 성능에 영향을 준 것으로 해석하였다.

- 본 실험을 통해 성능 병목은 단일 요소가 아니라, 데이터 크기 증가에 따라 GC와 직렬화 비용이 함께 증가하며 임계 구간에서 시스템 안정성을 결정한다는 것을 확인했다.
...
5. 본 실험에서 관측된 p95 붕괴는 DB/Hikari 지표보다 JVM allocation rate 증가와 GC safepoint 정지(STW) 누적과 더 강하게 상관관계를 보였다.
   ...
   [JFR/JMC 사용으로 병목 이동 확인 및 핫패스 개선 링크](./JFR/JMC를%20활용한%20Allocation%20기반%20성능%20병목%20분석.md)
   
 ## 최종 결과(Hot Path개선 -> 10% 성능 향상)  
| 구분                     | RPS    | P95(ms) | 평균 처리량(req/s, 3회 평균) | 실패율 |
   | ------------------------ | ------ | ------- | ---------------------------- | ------ |
